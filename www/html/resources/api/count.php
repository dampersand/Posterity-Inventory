...
//this function should also be called with a 'parameters' set of entries.  The function will then return a count of all documents in the index matching the parameters.
...
//unfortunately, the count API does not support (or, more accurately, ignores) the 'inner_hits' call used in search.php, so you cannot get a count of inner hits.  Use the search.php api call for that.
//Example search:
...
//this search would search the serials index for keywords matching '1test' and price greater than 10.  Additionally, it would only return a count of that ALSO have a history entry made by danpuser later than Jan 1, 2016 at noon and earlier than Feb 1, 2016 at noon.
...
if (isset($inputJSON['index'])) {
	$authData = auth($inputJSON, 'count.php');
} else {
  $authData = auth();
}
...
if (!isset($inputJSON['index']) || (!isset($inputJSON['parameters']))) {
  ...
  $logInput = [
    'flag' => 'BAD_INPUT',
    'call' => 'count.php',
    'subcall' => 'Not enough information when calling count.php',
    'user' => $authData['user']
  ];
